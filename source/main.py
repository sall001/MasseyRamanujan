...
def get_custom_bn_generator(args):
    ...
    if args.zeta_bn:
        ...
        if args.function_value is None:
            print('\'-function_value\' is required when using \'--integer_factorization_bn\'')
            exit(1)
        return series_generators.IntegerFactor(args.function_value, g_N_verify_terms), 1
...
def get_lhs_generator(generator_name, args):
    ...
    if generator_name == 'standard':
        if args.poly_deg <= 0 or not isinstance(args.coeff_lim, int) or args.coeff_lim <= 0:
            ...
            raise AttributeError
        return lhs_generators.create_standard_lhs(args.poly_deg, args.coeff_lim, args.out_dir,
...
def enumerate_over_gcf_main(args):
    ...
    if not args.lhs_constant:
        print(
            "You must input a parameter for the lhs_constant. Please run 'python main.py enumerate_over_gcf --help' to for more details about the required parameters.")
        return
